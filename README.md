...
- `simplePromptRandomization`: generic random script generator that runs for every simple prompt before Codex expands it into an image prompt.
...
Before Codex expands the instruction, the app creates an intermediate `randomPromptScript` with random content, composition, scene, mood, palette, viewpoint, and surprise detail. Routine-specific `promptVariation` slots are merged into that script when present.
