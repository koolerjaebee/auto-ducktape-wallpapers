...
- `naming.imageFilenamePattern`: wallpaper filename pattern. It may include `{promptSlug}` so Codex can derive a short name from the final prompt while the app still keeps `{targetId}` and `{timestamp}` for traceability.
...
- `simplePromptRandomization`: candidate menu and guardrails for Codex to write a fresh intermediate randomization script before expanding a simple prompt.
...
- `runCleanup.keepPreviousRunCount`: number of previous successful routine cycles to keep after a new wallpaper is applied. `0` keeps only the current cycle; `20` keeps the current cycle plus 20 previous cycles.
...
- `routines.demo.promptVariation`: routine-specific creative slots Codex can use so recurring wallpapers do not collapse into the same scene.
...
If `runCleanup.deletePreviousMacosWallpaperCachesOnSuccess` is enabled, macOS may ask whether `node` is allowed to access wallpaper cache data. To avoid repeated prompts, grant Full Disk Access to the Node executable used by the LaunchAgent:

1. Open System Settings > Privacy & Security > Full Disk Access.
2. Click `+`, then press `Cmd+Shift+G`.
3. Add the real Node path, for example `/opt/homebrew/Cellar/node/25.9.0_1/bin/node`.
4. If Node was upgraded through Homebrew, repeat this for the new Cellar path.
...
Codex first writes an intermediate randomization script from `simplePromptRandomization` and any routine-specific `promptVariation` slots, then expands the short instruction into a wallpaper-ready image prompt. When prompt slug naming is enabled, Codex also derives the filename slug from that final prompt.
